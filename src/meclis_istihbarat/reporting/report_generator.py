...
from meclis_istihbarat.core.database import (
    get_latest_profile,
    get_report_cache,
    save_report_cache,
    clear_report_cache
)
from meclis_istihbarat.scrapers.profile_scraper import get_weekly_comparison
from meclis_istihbarat.reporting.metrics import (
    ...
    def __init__(self, use_cache: bool = True, use_llm: bool = True):
        ...
        if use_llm:
            try:
                ...
                from meclis_istihbarat.analysis.analyzer import TweetAnalyzer
    ...
    def _generate_llm_analysis_section(self, username: str) -> str:
        ...
        try:
            ...
            if not result.get('validated') or not result.get('analysis'):
                return f"\n## 🕵️ Profesyonel İstihbarat Analizi\n\n{result.get('raw_response', '*Analiz yapılamadı*')}\n"
            ...
            # 1. Yeşil Takım
            report.append("\n### 🟢 Yeşil Takım (Parti Temsili ve Sadakat)")
            report.append(f"**Sadakat Düzeyi:** {analysis.loyalty_level}")
            report.append(f"\n**Analiz:** {analysis.green_summary}")

            # 2. Kırmızı Takım
            report.append("\n### 🔴 Kırmızı Takım (Rakip Analizi ve Eleştiri)")
            report.append(f"**Eleştiri Düzeyi:** {analysis.criticism_level}")
            report.append(f"\n**Analiz:** {analysis.red_summary}")

            # 3. Gri Takım
            report.append("\n### ⚪ Gri Takım (Bağımsız Alan ve Kurumsal Eleştiri)")
...
if __name__ == "__main__":
    ...
    if args.clear_cache:
        from meclis_istihbarat.core.database import clear_report_cache
