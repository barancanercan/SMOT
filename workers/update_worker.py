...
from database import init_database, clear_expired_cache
...
def update_embeddings(usernames=None):
    ...
    Args:
        usernames: Guncellenecek kullanicilar (None = hepsi)
    """
    from analysis.vector_db import rebuild_index

    print(f"\n{'='*60}")
    print(f"EMBEDDING GUNCELLEME")
    print(f"{'='*60}")
